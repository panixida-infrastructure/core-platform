...
Alertmanager sends Telegram notifications through a redundant Squid service. Squid prefers the WireGuard-backed Gluetun parent proxy and automatically uses the node's direct egress when that parent is unavailable. Synthetic checks monitor both the VPN-only and final egress routes; vmalert also watches Alertmanager delivery failures. Run the `Telegram Alerting Smoke Test` workflow to send one notification through each route and verify the corresponding Squid access-log hierarchy.
