...
Alertmanager sends Telegram notifications through a redundant Squid service and routes them to owner-specific forum topics: `tactical-heroes`, `dotnet-template`, `postgresql`, `core-platform`, and `observability`. Alerts without a known owner and synthetic tests use `unclassified-tests`. Squid prefers the WireGuard-backed Gluetun parent proxy and automatically uses the node's direct egress when that parent is unavailable. Synthetic checks monitor both the VPN-only and final egress routes; vmalert also watches Alertmanager delivery failures. Run the `Telegram Alerting Smoke Test` workflow to send one silent notification through each route and verify the corresponding Squid access-log hierarchy.
