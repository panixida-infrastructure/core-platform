...
target_excluded_databases="${TARGET_EXCLUDED_DATABASES:-default_db digital_event_manager dotnet_template}"
target_excluded_users="${TARGET_EXCLUDED_USERS:-gen_user digital_event_manager_user sonar dotnet_template_user}"
...
application_users_inventory="${BASH_SOURCE[0]%/*}/application-postgres-users.json"
dotnet_template_dev_user="$(jq -er '.users[] | select(.database == "dotnet_template_dev") | .login' "$application_users_inventory")"
dotnet_template_dev_password="$(secret_or_generate "$(jq -r '.DOTNET_TEMPLATE_DEV_DB_PASSWORD // empty' <<<"$applications_secret")")"
dotnet_template_prod_user="$(jq -er '.users[] | select(.database == "dotnet_template_prod") | .login' "$application_users_inventory")"
dotnet_template_prod_password="$(secret_or_generate "$(jq -r '.DOTNET_TEMPLATE_PROD_DB_PASSWORD // empty' <<<"$applications_secret")")"
tactical_heroes_dev_user="$(jq -er '.users[] | select(.database == "tactical_heroes_dev") | .login' "$application_users_inventory")"
tactical_heroes_dev_password="$(secret_or_generate "$(jq -r '.TACTICAL_HEROES_DEV_DB_PASSWORD // empty' <<<"$applications_secret")")"
tactical_heroes_prod_user="$(jq -er '.users[] | select(.database == "tactical_heroes_prod") | .login' "$application_users_inventory")"
